// ── Supabase Auth Module (Беларусь — таблицы _by) ──
...
// Профили — общая таблица (те же пользователи на обоих сайтах)
...
// ── Белорусские таблицы с суффиксом _by ──

async function saveQuizResult(score, total, timeMs) {
  const user = getUser();
  if (!user) return;
  return sbFetch('quiz_results_by', {
    method: 'POST',
...
async function savePuzzleResult(puzzleName, timeMs) {
  const user = getUser();
  if (!user) return;
  return sbFetch('puzzle_results_by', {
    method: 'POST',
...
function requireAuth() {
  if (!getUser() || !getToken()) {
    ...
    window.location.href = `auth_by.html?redirect=${current}`;
    return false;
  }
  return true;
}
